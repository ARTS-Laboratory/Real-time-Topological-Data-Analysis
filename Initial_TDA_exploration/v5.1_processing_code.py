...
plt.subplot(223)
plt.plot(x,y,'o-')
plt.xlabel('acceleratoin($t- 10$)')
plt.ylabel('acceleratoin (kg$_n$)')
plt.tight_layout()

plt.subplot(122)
tda_results = ripser.ripser(point_center_car,metric='manhattan')
...
plt.figure()
diagrams = ripser.ripser(point_center_car)['dgms']
persim.plot_diagrams(diagrams, show=True)

# plot the persistence diagram

H_0 = np.asarray(diagrams[0])

lines_e_sort = np.asarray(diagrams[0])[:,1]
